...
<template>
  ...
  >
    ...
    <div class="space-y-3">
      <p class="text-ga-gray-700">{{ definition }}</p>
      ...
      <div v-if="example" class="flex gap-2 text-sm">
        ...
        <p class="text-ga-gray-600 italic">{{ example }}</p>
      </div>

      <!-- Confusable term hint -->
      <div
        v-if="confusedWith && !compareMode"
        class="flex gap-2 text-sm p-2 bg-amber-50 rounded-md"
      >
        ...
        <div>
          ...
          <p v-if="distinction" class="text-amber-600 text-xs mt-1">
            {{ distinction }}
          </p>
        </div>
      </div>
    ...
    <template v-if="relatedTerms?.length" #footer>
      <div class="flex items-center gap-2 flex-wrap">
        <span class="text-xs text-ga-gray-600">Related:</span>
        <UBadge
