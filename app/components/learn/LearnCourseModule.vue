...
const { data: moduleData, pending, error } = await useFetch(`/api/course/${props.module.id}`)

// Mark module as started when viewing
onMounted(() => {
  markModuleStarted(props.module.id)
})

// Get previous and next modules
const currentIndex = computed(() => modules.findIndex(m => m.id === props.module.id))
...
const progress = computed(() => getModuleProgress(props.module.id))
const isCompleted = computed(() => progress.value.status === 'completed')

function handleMarkComplete() {
  markModuleCompleted(props.module.id)
}
  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
      <UButton
        ...
        @click="emit('navigate', previousModule.id)"
      >
      ...
      <UButton
        ...
        @click="emit('navigate', nextModule.id)"
      >
        {{ nextModule.title }}
      </UButton>
    </div>
