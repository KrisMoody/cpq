...
import type { TaxProfile } from '~/composables/useTaxProfiles'
...
const columns = [
  ...
  columnHelper.accessor('ratesCount', {
    header: 'Rates',
          ...
          <tr v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
            <th
              ...
              :class="[
                ...
                header.column.id === 'country' && 'w-[20%]',
                header.column.id === 'ratesCount' && 'w-[15%]',
            ...
            >
              <template v-if="cell.column.id === 'actions'">
                <div class="flex gap-1">
                  <UButton
                    :to="`/tax-profiles/${row.original.id}`"
                    variant="ghost"
                    size="xs"
              ...
              <template v-else-if="cell.column.id === 'ratesCount'">
                <UBadge v-if="(cell.getValue() as number) === 0" color="warning" variant="subtle">
                  No rates
                </UBadge>
                <span v-else>{{ cell.getValue() }} rate{{ (cell.getValue() as number) !== 1 ? 's' : '' }}</span>
              </template>
    ...
    <div class="md:hidden space-y-3">
      ...
      >
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 mt-3">
          <UBadge v-if="row.original.ratesCount === 0" color="warning" variant="subtle">
            No rates
          </UBadge>
          <span v-else class="text-sm">{{ row.original.ratesCount }} rate{{ row.original.ratesCount !== 1 ? 's' : '' }}</span>
          <UBadge :color="row.original.isActive ? 'success' : 'warning'" variant="subtle">
            {{ row.original.isActive ? 'Active' : 'Inactive' }}
          </UBadge>
        </div>
