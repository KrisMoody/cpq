  ...
  <div class="min-h-screen flex items-center justify-center bg-ga-gray-100 px-4">
    <div class="w-full max-w-md text-center">
      ...
      <div v-if="loading" class="space-y-4">
        <UIcon name="i-heroicons-arrow-path" class="w-8 h-8 text-ga-navy-500 animate-spin mx-auto" />
        <p class="text-ga-gray-600">Completing authentication...</p>
      </div>

      <!-- Error state -->
      <UCard v-else-if="error">
        <div class="space-y-4">
          <UIcon name="i-heroicons-exclamation-triangle" class="w-12 h-12 text-ga-red-500 mx-auto" />
          <h2 class="text-lg font-semibold">Authentication Failed</h2>
          <p class="text-ga-gray-600">{{ error }}</p>
