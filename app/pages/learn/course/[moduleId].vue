...
<template>
  <div v-if="currentModule" class="space-y-4">
    ...
    <div class="pb-4 border-b border-ga-gray-300">
      <div class="flex items-center gap-2 mb-2">
        <span class="text-sm font-mono text-ga-gray-600">Module {{ currentModule.number }}</span>
        <UBadge variant="subtle" size="xs">
          {{ currentModule.level }}
        </UBadge>
      </div>
      <h1 class="text-2xl font-bold">{{ currentModule.title }}</h1>
      <p class="text-ga-gray-600">{{ currentModule.focus }}</p>
    </div>
  ...
  <div v-else class="text-center py-12">
    <UIcon name="i-heroicons-exclamation-circle" class="w-12 h-12 text-ga-gray-500 mx-auto mb-4" />
    <h2 class="text-xl font-semibold mb-2">Module Not Found</h2>
    <p class="text-ga-gray-600 mb-4">The requested course module could not be found.</p>
